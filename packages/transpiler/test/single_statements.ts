...
describe("Single statements", () => {
  const tests = [
    ...
    {abap: "foo = 2.",                             js: "foo.set(abap.IntegerFactory.get(2));",                      skip: false},
    {abap: "foo = bar + 2.",                       js: "foo.set(abap.operators.add(bar,abap.IntegerFactory.get(2)));",             skip: false},
    {abap: "foo = bar - 2.",                       js: "foo.set(abap.operators.minus(bar,abap.IntegerFactory.get(2)));",           skip: false},
    ...
    {abap: "foo = CONV f( 1 ) + CONV f( 2 ).",     js: `foo.set(abap.operators.add(new abap.types.Float({qualifiedName: "F"}).set(abap.IntegerFactory.get(1)),new abap.types.Float().set(abap.IntegerFactory.get(2))));`, skip: false},
    ...
    // put the type name beside the operators and lose the left hand side
    ...
    {abap: "IF foo CP 'bar*'. ENDIF.",             js: "if (abap.compare.cp(foo, abap.CharacterFactory.get(4, 'bar*'))) {\n}",    skip: false},
    ...
    {abap: "INSERT 5 INTO tab INDEX sy-tabix.", js: "abap.statements.insertInternal({data: abap.IntegerFactory.get(5), index: abap.builtin.sy.get().tabix, table: tab});", skip: false},
    ...
    {abap: "foo->method( 1 ).",                       js: "await foo.get().method(abap.IntegerFactory.get(1));",             skip: true}, // todo, hmm, need to know the default parameter name?
    {abap: "foo->method( bar = 2 moo = 1 ).",         js: "await foo.get().method({bar: abap.IntegerFactory.get(2), moo: abap.IntegerFactory.get(1)});",       skip: false},
    {abap: "moo = foo->method( ).",                   js: "moo.set((await foo.get().method()));",              skip: false},
    {abap: "FORM foo. ENDFORM.",                      js: "async function foo(INPUT) {\n}\nabap.Forms['PROG-ZFOOBAR-FOO'] = foo;",                       skip: false},
    {abap: "PERFORM foo.",                            js: "await foo({});",                       skip: false},
    {abap: "PERFORM foo IN PROGRAM zsdfsd.",                        js: `await abap.Forms['PROG-ZSDFSD-FOO']({});`,                       skip: false},
    {abap: "PERFORM foo in program zsdfsd.",                        js: `await abap.Forms['PROG-ZSDFSD-FOO']({});`,                       skip: false},
    {abap: "PERFORM foo in program (var) changing moo.",            js: `await abap.Forms['PROG-' + var.get().trimEnd() + '-FOO']({"moo": moo});`,                       skip: false},
//    {abap: "PERFORM ('ASDF') IN PROGRAM ('ASDF') TABLES bar.",      js: `throw new Error("PerformTranspiler FormName not found");`,                       skip: false},
    ...
    {abap: "SPLIT |blah| AT '.' INTO lv_major lv_minor.", js: "abap.statements.split({source: new abap.types.String().set(`blah`), at: abap.CharacterFactory.get(1, '.'), targets: [lv_major,lv_minor]});",    skip: false},
  ...
  withKey: (i) => {return abap.compare.eq(i.field, abap.IntegerFactory.get(2));},
  withKeyValue: [{key: (i) => {return i.field}, value: abap.IntegerFactory.get(2)}],
  usesTableLine: false,
  withKeySimple: {"field": abap.IntegerFactory.get(2)}});`, skip: false},
  ...
  withKeySimple: {"program_ndx": lv_program_ndx}});`,   skip: false},
    ...
    {abap: "type->type_kind = 2.",                    js: "type.get().type_kind.set(abap.IntegerFactory.get(2));",         skip: false},
    ...
    {abap: "FIND FIRST OCCURRENCE OF REGEX 'b+c' IN 'abcd' MATCH COUNT lv_cnt MATCH LENGTH lv_len.",
      js: "abap.statements.find(abap.CharacterFactory.get(4, 'abcd'), {regex: abap.CharacterFactory.get(3, 'b+c'), first: true, count: lv_cnt, length: lv_len});", skip: false},
    {abap: "FIND REGEX '11(\\w+)22' IN '11abc22' SUBMATCHES lv_host.",
      js: "abap.statements.find(abap.CharacterFactory.get(7, '11abc22'), {regex: abap.CharacterFactory.get(9, '11(\\\\w+)22'), submatches: [lv_host]});", skip: false},
    {abap: "SHIFT lv_bitbyte LEFT DELETING LEADING '0 '.", js: `abap.statements.shift(lv_bitbyte, {direction: 'LEFT',deletingLeading: abap.CharacterFactory.get(2, '0 ')});`, skip: false},
    {abap: "SHIFT lv_temp BY 1 PLACES LEFT.", js: `abap.statements.shift(lv_temp, {direction: 'LEFT',places: abap.IntegerFactory.get(1)});`, skip: false},
    {abap: "SHIFT lv_temp UP TO '/' LEFT.", js: `abap.statements.shift(lv_temp, {direction: 'LEFT',to: abap.CharacterFactory.get(1, '/')});`, skip: false},
    ...
    {abap: "foo = 2 ** 2.",   js: `foo.set(abap.operators.power(abap.IntegerFactory.get(2),abap.IntegerFactory.get(2)));`,                skip: false},
    {abap: "foo = 5 DIV 2.",  js: `foo.set(abap.operators.div(abap.IntegerFactory.get(5),abap.IntegerFactory.get(2)));`,                  skip: false},
    {abap: "foo+5(1) = 'A'.", js: `new abap.OffsetLength(foo, {offset: 5, length: 1}).set(abap.CharacterFactory.get(1, 'A'));`, skip: false},
    {abap: "foo(1) = 'a'.",   js: "new abap.OffsetLength(foo, {length: 1}).set(abap.CharacterFactory.get(1, 'a'));",            skip: false},
    {abap: "foo+1 = 'a'.",    js: "new abap.OffsetLength(foo, {offset: 1}).set(abap.CharacterFactory.get(1, 'a'));",            skip: false},
    {abap: "foo+1(1) = 'a'.", js: "new abap.OffsetLength(foo, {offset: 1, length: 1}).set(abap.CharacterFactory.get(1, 'a'));", skip: false},
    {abap: "foo(bar) = 'a'.", js: "new abap.OffsetLength(foo, {length: bar}).set(abap.CharacterFactory.get(1, 'a'));",    skip: false},
    {abap: "IF iv_cd = '' OR iv_cd = '.'.\nENDIF.", js: "if (abap.compare.eq(iv_cd, abap.CharacterFactory.get(1, '')) || abap.compare.eq(iv_cd, abap.CharacterFactory.get(1, '.'))) {\n}", skip: false},
    {abap: "TRY. ENDTRY.", js: ``,    skip: false, only: false},
    {abap: "MESSAGE e058(00) WITH 'Value_1' 'Value_2' 'Value_3' 'Value_4' INTO lv_dummy.",
      js: `await abap.statements.message({into: lv_dummy, id: "00", number: "058", type: "E", with: [abap.CharacterFactory.get(7, 'Value_1'),abap.CharacterFactory.get(7, 'Value_2'),abap.CharacterFactory.get(7, 'Value_3'),abap.CharacterFactory.get(7, 'Value_4')]});`, skip: false},
    {abap: "MESSAGE ID sy-msgid TYPE 'S' NUMBER sy-msgno WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4 INTO rv_text.",
      js: `await abap.statements.message({into: rv_text, id: abap.builtin.sy.get().msgid, type: abap.CharacterFactory.get(1, 'S'), number: abap.builtin.sy.get().msgno, with: [abap.builtin.sy.get().msgv1,abap.builtin.sy.get().msgv2,abap.builtin.sy.get().msgv3,abap.builtin.sy.get().msgv4]});`, skip: false},
    ...
    {abap: "SELECT * FROM t100 INTO TABLE lt_result UP TO 2 ROWS.",
      js: `await abap.statements.select(lt_result, {select: "SELECT * FROM " + abap.buildDbTableName("t100") + " UP TO " + abap.IntegerFactory.get(2).get() + " ROWS"});`},
    ...
    {abap: "GET REFERENCE OF blah INTO ref.", js: `abap.statements.getReference(ref, blah);`, skip: false},
    {abap: `GET REFERENCE OF <item> INTO lr_stack_top.`, js: `abap.statements.getReference(lr_stack_top, fs_item_);`, skip: false},
    ...
    {abap: "COMMIT WORK.",     js: `await abap.statements.commit();`, skip: false},
    {abap: "COMMIT WORK AND WAIT.", js: `await abap.statements.commit({"wait": true});`, skip: false},
    {abap: "COMMIT CONNECTION mycon.", js: `await abap.statements.commit({"connection": "MYCON"});`, skip: false},
    {abap: "COMMIT CONNECTION 'mycon'.", js: `await abap.statements.commit({"connection": "MYCON"});`, skip: false},
    {abap: "COMMIT CONNECTION (mycon).", js: `await abap.statements.commit({"connection": mycon.get().trimEnd().toUpperCase()});`, skip: false},
    {abap: "ROLLBACK CONNECTION mycon.", js: `await abap.statements.rollback({"connection": "MYCON"});`, skip: false},
    {abap: "ROLLBACK CONNECTION (mycon).", js: `await abap.statements.rollback({"connection": mycon.get().trimEnd().toUpperCase()});`, skip: false},
    {abap: `INSERT 1 INTO TABLE tab.`, js: `abap.statements.insertInternal({data: abap.IntegerFactory.get(1), table: tab});`, skip: false},
    {abap: "ROLLBACK WORK.",   js: `await abap.statements.rollback();`, skip: false},
...
await abap.Classes['KERNEL_CALL_TRANSFORMATION'].call({name: "id",sourceXML: iv_string,options: {value_handling:abap.CharacterFactory.get(16, 'accept_data_loss')},result: {data:rs_xml}});`},
    {abap: `CALL TRANSFORMATION id
...
await abap.Classes['KERNEL_CALL_TRANSFORMATION'].call({name: "id",resultXML: rv_xml,options: {initial_components:abap.CharacterFactory.get(8, 'suppress')},source: {data:is_data}});`},
    {abap: `CALL TRANSFORMATION id
      OPTIONS value_handling = 'accept_data_loss'
...
await abap.Classes['KERNEL_CALL_TRANSFORMATION'].call({name: "id",sourceXML: mi_xml_doc,options: {value_handling:abap.CharacterFactory.get(16, 'accept_data_loss')},result: (lt_rtab)});`},
    {abap: `CALL TRANSFORMATION id
      OPTIONS initial_components = 'suppress'
      SOURCE (lt_stab)
...
await abap.Classes['KERNEL_CALL_TRANSFORMATION'].call({name: "id",resultXML: li_doc,options: {initial_components:abap.CharacterFactory.get(8, 'suppress')},source: (lt_stab)});`},
    {abap: `CALL TRANSFORMATION id
SOURCE
  data   = <fs>
  fields = fields
...
await abap.Classes['KERNEL_SCAN_ABAP_SOURCE'].call({scan_abap_source: source, tokens_into: tokens, statements_into: statements, with_analysis: true, with_comments: true, with_pragmas: abap.CharacterFactory.get(1, '*')});`},
...
await abap.Classes['KERNEL_PUSH_CHANNELS'].wait({seconds: abap.IntegerFactory.get(10),cond: () => {return abap.compare.initial(lo_handler.get().message) === false;}});`},
    {abap: `ADD 2 to foo.`,
      js: `foo.set(abap.operators.add(foo,abap.IntegerFactory.get(2)));`},
    {abap: `MULTIPLY foo by 2.`,
      js: `foo.set(abap.operators.multiply(foo,abap.IntegerFactory.get(2)));`},
    {abap: `DIVIDE foo by 2.`,
      js: `foo.set(abap.operators.divide(foo,abap.IntegerFactory.get(2)));`},
  ...
  fs_fs_.assign(unique1);
    ...
    {abap: `LOOP AT table ASSIGNING <n> USING KEY array_index WHERE path = 'hello1'. ENDLOOP.`,
      js: `for await (const unique1 of abap.statements.loop(table,{usingKey: "array_index",where: async (I) => {return abap.compare.eq(I.path, abap.CharacterFactory.get(6, 'hello1'));},topEquals: {"path": abap.CharacterFactory.get(6, 'hello1')}})) {
...
await abap.Classes['KERNEL_AUTHORITY_CHECK'].call({object: obj,user: usr,fields: [{id: id1, field: val1}, {id: id2, dummy: true}]});`},
  ...
  if (e.classic) {
    ...
    {abap: "NEW-PAGE NO-TITLE LINE-SIZE 80.", js: `throw new Error("NEW-PAGE, not supported, transpiler");`, skip: false},
    {abap: "HIDE gv_id.", js: `throw new Error("HIDE, not supported, transpiler");`, skip: false},
